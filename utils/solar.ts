...
// Returns sun altitude in degrees for a given date/time, location AND timezone
export const getSunPosition = (date: Date, lat: number, lng: number, tzOffset: number) => {
  // Day of year
  const d = Math.floor((date.getTime() - new Date(date.getFullYear(), 0, 0).getTime()) / 1000 / 60 / 60 / 24);

  // Equation of Time (EOT)
  // B = 360 * (d - 81) / 365
  const b = degToRad(360 * (d - 81) / 365);
  const equationOfTime = 9.87 * Math.sin(2 * b) - 7.53 * Math.cos(b) - 1.5 * Math.sin(b);
  
  // Time Correction = 4 * (Longitude - 15 * Timezone) + EOT
  const timeCorrection = 4 * (lng - (15 * tzOffset)) + equationOfTime;
  
  // Solar Time
  const localTimeMinutes = date.getHours() * 60 + date.getMinutes();
  const solarTimeMinutes = localTimeMinutes + timeCorrection;
  
  // Hour Angle (ha): 0 at Solar Noon
  const ha = (solarTimeMinutes / 60 - 12) * 15;

  const sinAlt = Math.sin(degToRad(lat)) * Math.sin(degToRad(23.45 * Math.sin(b))) + 
                 Math.cos(degToRad(lat)) * Math.cos(degToRad(23.45 * Math.sin(b))) * Math.cos(degToRad(ha));
