...
export const generateArduinoSketch = (config: AppConfig): string => {
  ...
  // Calculamos valores iniciales por defecto para inyectar en el código C++
  const defLat = location.lat;
  const defLon = location.lng;

  return `/*
  CONTROLADOR DE VENTANA CIRCADIANA (Luminia V4.8 - Solar Precision)
  -----------------------------------------------
  
  MEJORA V4.8:
  - Implementada Ecuacion del Tiempo (EOT) y Correccion de Longitud.
  - Corrige el desfase de ~1h-1.5h entre el mediodia solar y el reloj.
  
  IMPORTANTE: 
  Asegurate de que conf.timezone coincide con tu hora real (Invierno/Verano).
  
  COMANDOS BASICOS:
  STATUS, SAVE, RESET, SETTIME:YYYYMMDDHHMM
  TZ:X -> Configurar Huso Horario (Ej: TZ:1, TZ:2, TZ:-5)
*/
...
struct Config {
  ...
  float kFullAlt;
  float warmBias;
  float timezone;       // NUEVO V4.8: UTC Offset
};

Config conf;
const byte MAGIC_NUMBER = 0x48; // V4.8 Force Reset
...
void loadConfig() {
  EEPROM.get(0, conf);
  // Si la version no coincide, cargamos defaults (Hard Reset de configuración)
  if (conf.magic != MAGIC_NUMBER) {
    Serial.println(F("Actualizando firmware a V4.8... Cargando defaults."));
    ...
    conf.timezone = ${timezoneOffset}; // Valor inicial del frontend
...
void printStatus() {
  ...
  bt.println(F("--- ESTADO LUMINIA V4.8 ---"));
  bt.print(F("HORA: ")); 
...
void processCommand(String cmd) {
  ...
  // --- PARAMETROS ---
  ...
  else if (cmd.startsWith("TZ:")) { conf.timezone = cmd.substring(3).toFloat(); bt.println("OK"); }
    ...
    if (cmd.length() > 0) bt.println("CMD ??");
  }
}
...
void setup() {
  ...
  loadConfig();
  Serial.println("Luminia V4.8 Iniciado - SOLAR PRECISION");
  bt.println("LUMINIA V4.8 READY");
}
...
float getSunAltitude(DateTime t) {
  ...
  // 1. CALCULO ECUACION DEL TIEMPO (EOT)
  // B = 360 * (d - 81) / 365
  float rad = 0.01745329;
  float bRad = 2.0 * 3.14159 * (d - 81) / 365.0;
  
  // EOT (minutos) = 9.87 sin(2B) - 7.53 cos(B) - 1.5 sin(B)
  float eot = 9.87 * sin(2*bRad) - 7.53 * cos(bRad) - 1.5 * sin(bRad);
  
  // 2. CORRECCION DE LONGITUD Y HUSO HORARIO
  // Correccion (min) = 4 * (Lon - 15 * Timezone) + EOT
  float timeCorrection = 4 * (conf.lon - (15 * conf.timezone)) + eot;
  
  // 3. TIEMPO SOLAR VERDADERO
  float localMinutes = t.hour() * 60 + t.minute();
  float solarMinutes = localMinutes + timeCorrection;
  
  // Angulo Horario (HA): 0 en Mediodia Solar
  float ha = (solarMinutes / 60.0 - 12.0) * 15.0;
  
  // 4. DECLINACION
  float decl = 23.45 * sin(bRad);
  
  // 5. ALTITUD SOLAR (Formula esferica)
  float latRad = conf.lat * rad;
  float declRad = decl * rad;
  float haRad = ha * rad;

  float sinAlt = sin(latRad) * sin(declRad) + cos(latRad) * cos(declRad) * cos(haRad);
  
  return asin(sinAlt) * 57.29577; // Grados
}
